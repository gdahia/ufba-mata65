...
function initGUI() {
  params = {zBuffer: true, faceCulling: "FrontFace"};

  var gui = new dat.GUI();
  gui.add(params, 'zBuffer').onChange(function() { guiChanged = true; });
  gui.add(params, 'faceCulling', ["FrontFace", "BackFace", "DoubleSide"])
      .onChange(function() { guiChanged = true; });
  gui.open();
};
...
function render() {
  ...
  if (guiChanged) {
    ...
    if (zBuffer !== params.zBuffer) zBuffer = params.zBuffer;

    if (faceCull !== params.faceCulling) faceCull = params.faceCulling;
